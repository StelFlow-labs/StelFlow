...
This repo now has a root [package.json](../package.json) (added by this same PR, for the docs
lint/format tooling in [.github/workflows/docs.yml](../.github/workflows/docs.yml)), pinning
`packageManager` to a specific pnpm version. `pnpm install` against it, run for real:
...
Corepack reads `packageManager` from `package.json` and fetches the pinned pnpm version
automatically the first time you run a `pnpm` command in this repo — you don't need to `npm
install -g pnpm` yourself.
